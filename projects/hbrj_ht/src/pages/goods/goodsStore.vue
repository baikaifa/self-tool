<template >
  <div class="goodsStore" ref="me" id="goodStore">
    <!-- 头部切换tab -->
    ...
    <div class="goodsList">
      <div class="btnsList textL mb">
        ...
        <el-button @click="putItemsToshelf" type="primary" class="btns">
          <i class="iconfont icon-piliangshangjia mr5"></i>批量上架
        </el-button>

        <el-button @click="exportGoods" type="primary" class="btns">
          <i class="iconfont icon-daochu mr5"></i>导出
        </el-button>
        <el-upload style="margin-left:10px;" :data="{'sid':sid}" action="/item/importexcel">
          <el-button :on-change="handleChange" type="primary">
            <i class="iconfont icon-Import mr5"></i>导入
          </el-button>
      ...
      <el-table border :data="tableData" @selection-change="selectionChange">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <div
              class="redColor"
              @click="putToshelf(scope.$index, scope.row)"
...
export default {
  data() {
    return {
      ...
      pageNo: 1,
      tabList: [
        { name: "商品库", path: "goodsStore" },
        { name: "商品品类库", path: "category" },
        { name: "商品回收站" },
        { name: "商品拉取规则" }
      ],
      goods: {
  ...
  mounted() {
    // console.log(this.$refs.me.scrollTop);
    // setInterval(() => {
    //   console.log(this.$refs.me.parentNode.scrollTop);
    // }, 1000);
    console.log(document.getElementById('goodStore').parentNode);
    this.$refs.me.parentNode.scrollTop=this.scrollTop;
  },
  beforeRouteLeave(to, from, next) {
    this.scrollTop=this.$refs.me.parentNode.scrollTop
    next();
  },
  beforeRouteEnter(to, from, next) {
    next(vm => {
      document.getElementById('goodStore').parentNode.scrollTop = vm.scrollTop
    });
  },
  created() {
