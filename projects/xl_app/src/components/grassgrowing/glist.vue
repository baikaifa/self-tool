<template>
	<scroller :on-refresh="refList" :on-infinite="lodList" ref="mallScro" :noDataText="noDaTxt">
		...
		<div class="goods" ref="goodsList">
			<div class="goods_inner" v-for="(item,index) in items" :key="index" @click.stop="getCommInfo(item)" :id="index">
				<img class="goods_img" modeType="event" v-lazy="item.imgUrl" />
...
export default {
	...
	methods: {
		 ...
		 */
		getCommInfo(item) {
			this.$router.push({name:'postDetail',params:{tid:item.tid,userId:item.userInfo.uid}})
		},
		getMoreSame(img) {
			this.$router.push({
				name: "shareList",
...
.goods_inner .xinRed{color:#ff2741;}
</style>